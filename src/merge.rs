...
pub fn keep_intervals(
	...
	let joined_groups       = segments_groups.join(&data_groups, &join_left, &join_left, JoinType::Inner)?;
	...
	let k:Vec<_> = (0..10).map(|index|{
		...
		let poop:() = segments_rows.u32().unwrap().map(|segment_index|{
			...
			if dat.width()*dat.height() > 0{
				println!("=================");
				println!("For target index {:?} from slk {:?} to slk {:?}",index, segment_slk_from, segment_slk_to);
